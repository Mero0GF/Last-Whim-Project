    ...
    private void FixedUpdate()
    {
        switch (state)
        {
            ...
            case State.Attack:
                ...
                int localMask = LayerMask.GetMask("Col");
                RaycastHit2D collision = Physics2D.Raycast(swordPos, atkDir, speed * Time.deltaTime, localMask);
                if (collision == true)
                {
                    //rb.position = rb.position + atkDir * speed * Time.deltaTime;
